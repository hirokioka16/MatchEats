...
public interface FoodRepository extends JpaRepository<FoodTblEntity,Integer>{
	...
	@Query("SELECT f FROM FoodTblEntity f left join f.userTbl u WHERE u.userId = :userId AND f.eatFlag != 2")
	...
	//食べたいもの検索用
	@Query("SELECT f FROM FoodTblEntity f WHERE f.foodName LIKE %:keyword% AND f.eatFlag=0")
